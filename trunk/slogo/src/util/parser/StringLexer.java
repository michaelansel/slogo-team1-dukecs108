...
public class StringLexer extends AbstractLexer {
	...
	public List<Token> tokenize() {
		if (getTokenRules() == null)
			throw new RuntimeException("TokenRules never initialized!");
		StringBuilder remainder = new StringBuilder(getInput());
		List<Token> results = new ArrayList<Token>();
		while (remainder.length() > 0) {
			List<Token> tokens = new ArrayList<Token>();
			for (ITokenRule rule : getTokenRules()) {
				Matcher matcher = ((StringTokenRule) rule).getPattern()
			...
			for (Token match : tokens)
				if (((String) match.value).length() > ((String) bestMatch.value)
						.length())
					bestMatch = match;
