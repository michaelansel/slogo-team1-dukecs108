...
public class GrammarParserFactory {
	...
	private Map<String, ParseTreeNode> buildParseTrees(ResourceBundle syntax)
		...
		Map<String, ParseTreeNode> parseTrees = new HashMap<String, ParseTreeNode>();
		for (String ruleName : syntax.keySet()) {
			GrammarParser parser = new GrammarParser(new GrammarLexer(
					syntax.getString(ruleName)));
			ParserResult parsedGrammar = parser.run();
			if (parsedGrammar.getList().size() != 1)
				throw new ParserException("Improper ParserResult: "
						+ parsedGrammar.toString());
			if (parsedGrammar.getList().get(0) instanceof ParseTreeNode) {
				parseTrees.put(ruleName, (ParseTreeNode) parsedGrammar
						.getList().get(0));
			} else
				throw new RuntimeException("Invalid result: "
						+ parsedGrammar.getList().get(0).toString());
		}
		return parseTrees;
	}
