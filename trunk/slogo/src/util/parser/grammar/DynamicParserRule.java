...
final class DynamicParserRule extends AbstractParserRule {
	...
	DynamicParserRule(AbstractParser parser, String ruleName,
	...
	private Method getMethod(String name, List<Object> parameters) {
		...
		if (matching_methods.isEmpty())
			throw new IllegalArgumentException("ParserRule not found: " + name);
		if (matching_methods.size() == 1)
			return matching_methods.get(0);
	...
	public void initializeRule() {
		...
		for (ParseTreeNode child : this.children)
			if (this.definedRules.containsKey(child.getName()))
				params.add(this.definedRules.get(child.getName()));
			else
				try {
		...
		AbstractParserRule[] paramsArray = new AbstractParserRule[params.size()];
		...
		try {
			...
			throw new RuntimeException("Error building ParserRules: "
					+ this.ruleName + " :: " + params.toString() + e.toString());
		}
	}
}
