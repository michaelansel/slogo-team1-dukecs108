...
public class GrammarParser extends AbstractParser {
	...
	// {
	// Token tok = (Token) o;
	// if (tok.tokenRule == GrammarLexer.Token.Constant) parameters.add((String)
	// tok.value);
	...
	// ((Token) matchList.get(0)).tokenRule == GrammarLexer.Token.Whitespace)
	// matchList.remove(0);
	// if (!(matchList.size() == 1 && matchList.get(0) instanceof
	// ParseTreeNode)) throw new
	// ParserException("Invalid input to result processor. Remaining: " +
	// matchList.toString());
	...
	public AbstractParserRule Constant = new AbstractParserRule() {
		...
		protected ParserResult processResult(ParserResult result)
			...
			if (result.getList().size() != 1)
				parseError("Invalid parse result!");
			return new ParserResult(new ParseTreeNode(
					(String) ((Token) (result.getList().get(0))).value));
		}
	};
	...
	public AbstractParserRule SimpleRule = new AbstractParserRule() {
		...
		public void initializeRule() {
			setRule(Sequence(
					...
					ZeroOrMore(Optional(GrammarLexer.Token.Whitespace),
							GrammarLexer.Token.Delimiter,
							Optional(GrammarLexer.Token.Whitespace), Expression),
					Optional(GrammarLexer.Token.Whitespace),
					GrammarLexer.Token.EndGroup));
		}
		...
		protected ParserResult processResult(ParserResult result)
			...
			if (list.size() < 4)
				throw new RuntimeException("Invalid parse result!");
			String name = (String) ((Token) (list.remove(0))).value; // Constant
																		// ==
																		// Rule
																		// Name
			...
			for (Object o : list)
				if (o instanceof ParseTreeNode)
					params.add((ParseTreeNode) o);
				else if (o instanceof Token
						&& ((((Token) o).tokenRule == GrammarLexer.Token.Delimiter) || (((Token) o).tokenRule == GrammarLexer.Token.Whitespace)))
					continue;
				else
					throw new RuntimeException("Bad result object: "
							+ o.toString() + "\n  " + result.toString()
	...
	public AbstractParserRule Token = new AbstractParserRule() {
		...
		public void initializeRule() {
			setRule(Sequence(GrammarLexer.Token.BeginToken,
					GrammarLexer.Token.Constant, GrammarLexer.Token.EndToken));
		}
