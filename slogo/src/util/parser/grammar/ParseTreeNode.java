...
public class ParseTreeNode implements Cloneable {
	...
	public AbstractParserRule toParserRule(AbstractParser parser,
		...
		if (myParserRule != null)
			return myParserRule; // Short-circuit for already-resolved rules

		if (myType == Type.Token) {
			ITokenRule tokenRule = parser.getLexer()
					.getTokenRuleByName(myToken);
			if (tokenRule == null)
				throw new ParserException("Couldn't find token in lexer: "
						+ myToken);
			return parser.ExactlyOne(tokenRule);
		}
		if (myType == Type.IndirectCall && definedRules.containsKey(getName())
				&& definedRules.get(getName()) != null) {
			// rule already resolved
			return definedRules.get(getName());
		}
		return (myParserRule = new DynamicParserRule(parser, getName(),
				myParameters, definedRules));
	}

	@Override
	public String toString() {
		if (myType == Type.Token)
			return String.format("ParseTreeNode(%s(%s))", getName(), myToken);
		else if (myParameters != null)
			return String.format("ParseTreeNode<%s(%s)>", getName(),
					myParameters.toString());
		else
			return String.format("ParseTreeNode<%s()>", getName());
	}
}
