...
public class PanelFactory {
	...
	public static JPanel createPanel(String classNameString) {
		try {
			...
			Class className = Class.forName(resources
					.getString(classNameString));
			JPanel obj = (JPanel) className.newInstance();

			// Create panel
			int[] size = resources.getIntegerArray(classNameString + "Size",
					"x");
			int[] location = resources.getIntegerArray(classNameString
					+ "Location");
			String[] border = resources.getStringArray(classNameString
					+ "Border");

			obj.setBounds(location[0], location[1], size[0], size[1]);
			obj.setBorder(BorderFactory.createLineBorder(
					getColorByName(border[1]), Integer.parseInt(border[0])));
			obj.setBackground(getColorByName(resources
					.getString(classNameString + "BkgColor")));
	...
	private static Color getColorByName(String fieldNameString) {
		try {
			Color col = (Color) (Color.class).getField(fieldNameString).get(
					null);
			return col;
